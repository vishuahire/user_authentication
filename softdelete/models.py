...
class SoftDeleteHelper():
    ...
    def get_un_soft_deleted_objects(self, collector):
        ...
        for model, instances in collector.data.items():
            try:
                if model._meta.get_field("is_active") == True:
                    # print(model._meta.get_field("is_active"))
                    collector.data[model] = set(
                        filter(lambda x: not x.is_active, instances))
    ...
    def sql_model_wise_batch_update(self, model, instances, is_active=True):
        query = sql.UpdateQuery(model)
       
        query.update_batch([obj.pk for obj in instances],
                           {'is_active': is_active}, self.using)
        ...
        if not model._meta.auto_created:
            for obj in instances:
                if signal_type.__contains__('save'):
                    getattr(signals, signal_type).send(sender=model,
                                                       instance=obj,
                                                       created=False,
                                                       using=self.using)
                else:
    ...
    def do_work(self, objs):
        ...
        for model, instances in six.iteritems(collector.data):
            ...
            try:
                if self.delete_type == 'soft_delete':
                    self.sql_model_wise_batch_update(model,
                                                     instances,
                                                     is_active=False)
                else:
                    self.sql_model_wise_batch_update(model,
